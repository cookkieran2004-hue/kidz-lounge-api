...
const LEDGER_TYPES = new Set(['PTO', 'UPTO']);
...
async function adjustBalance(q, {
  ...
  // The ledger covers PTO and UPTO. Requests from before the PTO/UPTO
  // redesign (Vacation/Sick/Personal) still move their old, retired
  // balances as they always did, just without a ledger entry.
  if (LEDGER_TYPES.has(type) && await hasLedgerTable(q)) {
    await q.query(
...
// Takes back what an approved request took off the balance, as if it had
// never been approved: the hours go back on the balance and its "used"
// entries are removed from the history (no refund entry). Returns false --
// changing nothing -- when there's no "used" entry to remove (older
// requests approved before the ledger), so the caller records a refund.
async function removeUsage(q, { username, type, requestId }) {
  if (!LEDGER_TYPES.has(type) || !(await hasLedgerTable(q))) return false;
  const used = (await q.query(
    `SELECT id, hours FROM "TimeOffLedger" WHERE username=$1 AND balance_type=$2 AND kind='used' AND request_id=$3`,
    [username, type, String(requestId)]
  )).rows;
  if (!used.length) return false;
  const giveBack = round2(-used.reduce((s, e) => s + Number(e.hours), 0));
  const row = (await q.query(
    'SELECT balance_hours FROM "TimeOffBalances" WHERE username=$1 AND balance_type=$2 FOR UPDATE',
    [username, type]
  )).rows[0];
  await q.query(
    `INSERT INTO "TimeOffBalances" (username, balance_type, balance_hours)
     VALUES ($1, $2, $3)
     ON CONFLICT (username, balance_type) DO UPDATE SET balance_hours = $3, updated_at = now()`,
    [username, type, round2((row ? Number(row.balance_hours) : 0) + giveBack)]
  );
  await q.query('DELETE FROM "TimeOffLedger" WHERE id = ANY($1)', [used.map(e => e.id)]);
  return true;
}
...
module.exports = { adjustBalance, removeUsage, hasLedgerTable, inTransaction, runWeeklyPtoAccrual, periodsSince, applyCarryover };
