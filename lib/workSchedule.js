...
// A provider's day with no patients seen -- nothing booked, or everything
// canceled, a no-show or a hold -- counts as 0 hours worked.
...
const NO_WORK_STATUSES = new Set(['Canceled', 'No Show', '*HOLD*']);
// The placeholder "patient" booked to hold time (HOLD_PATIENT_NAME in the
// frontend's SchedulePage.jsx) -- not a patient seen.
const HOLD_PATIENT_NAME = 'hold - see comments';
const patientSeen = (a) => !NO_WORK_STATUSES.has(a.appointment_status)
  && String(a.patient_name || '').trim().toLowerCase() !== HOLD_PATIENT_NAME;
...
function estimateDay(schedule, dateStr, requests, appointments) {
  ...
  // A provider who saw no patients that day didn't work it.
  const seen = (appointments || []).filter(a => a.appointment_date === dateStr && patientSeen(a));
  if (schedule.kind === 'provider' && seen.length === 0) {
    if (day.scheduled > day.off) day.notes.push('No patients seen');
    day.noPatients = true;
    return day;
  }
