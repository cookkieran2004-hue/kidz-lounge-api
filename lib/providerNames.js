...
async function assertProviderNotLinkedElsewhere(client, providerName, exceptStaffId = null) {
  ...
  const res = await client.query(
    'SELECT username FROM "Staff" WHERE provider_name=$1 AND archived=false AND ($2::int IS NULL OR id <> $2::int)',
    [providerName, exceptStaffId]
  );
  if (res.rows[0]) {
    throw new HttpError(409, `${providerName} is already linked to another account (${res.rows[0].username}).`);
  }
}
